...
func (r *GameRepository) GetGameByTitleAndReleaseYear(ctx context.Context, title string, releaseYear int32) (*model.Game, error) {
	...
	var game model.Game
	gameRow := r.conn.Get(ctx, getGameQuery, title, releaseYear)
	err := gameRow.Scan(
		&game.GameID,
		&game.Title,
		&game.Description,
	...
	genreRows, err := r.conn.Select(ctx, getGameGenresQuery, game.GameID)
	if err != nil {
	...
	tagRows, err := r.conn.Select(ctx, getGameTagsQuery, game.GameID)
	if err != nil {
