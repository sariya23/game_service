...
func TestDeleteGame(t *testing.T) {
	t.Run("Успешное удаление игры", func(t *testing.T) {
		...
		suite.gameMockRepo.On("DaleteGame", mock.Anything, gameID).Return(dto.DeletedGameFromGame(deletedGame), nil).Once()
		suite.s3Mock.On("DeleteObject", mock.Anything, gameKey).Return(nil).Once()

		deletedGameID, err := suite.gameService.DeleteGame(context.Background(), gameID)
	...
	t.Run("Нет игры для удаления", func(t *testing.T) {
		suite := NewSuite()
		gameID := uint64(4)
		suite.gameMockRepo.On("DaleteGame", mock.Anything, gameID).Return(nil, outerror.ErrGameNotFound).Once()
		deletedGameID, err := suite.gameService.DeleteGame(context.Background(), gameID)
	...
	t.Run("Неожиданная ошибка при удалении игры", func(t *testing.T) {
		...
		suite.gameMockRepo.On("DaleteGame", mock.Anything, gameID).Return(nil, someErr).Once()
		deletedGameID, err := suite.gameService.DeleteGame(context.Background(), gameID)
	...
	t.Run("Не удалось удалить обложку из S3", func(t *testing.T) {
		...
		gameKey := minioclient.GameKey(deletedGame.Title, int(deletedGame.ReleaseDate.Year()))
		suite.gameMockRepo.On("DaleteGame", mock.Anything, gameID).Return(dto.DeletedGameFromGame(deletedGame), nil).Once()
		suite.s3Mock.On("DeleteObject", mock.Anything, gameKey).Return(someErr).Once()
		deletedGameID, err := suite.gameService.DeleteGame(context.Background(), gameID)
		require.Equal(t, deletedGame.GameID, deletedGameID)
		require.ErrorIs(t, err, someErr)
	})
